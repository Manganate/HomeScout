...
def test_haversine_known_distance():
    """Calgary CBD to Edmonton is roughly 280 km."""
    d = crit.haversine_km(CBD_LAT, CBD_LON, 53.5444, -113.4909)
    assert 270 < d < 290, d


def test_haversine_zero():
    assert crit.haversine_km(51.0, -114.0, 51.0, -114.0) == pytest.approx(0.0, abs=1e-9)
...
def test_far_listing_rejected_by_geo_bound(c):
    # Edmonton — far outside any Calgary commute.
    reason = crit.check(_listing(latitude=53.5444, longitude=-113.4909), c)
