...
def _result(**overrides):
    base = {
        "MlsNumber": "A2100001",
        "RelativeDetailsURL": "/real-estate/12345678/123-main-st-sw-example-city",
        ...
        "Property": {
            ...
            "Address": {
                "AddressText": "123 Main St SW, Example City, Alberta T2P1A1",
                "Latitude": "45.0000",
                "Longitude": "-95.0000",
            },
        },
...
def test_parses_a_complete_record(source):
    ...
    assert listing.latitude == pytest.approx(45.0)
    assert listing.longitude == pytest.approx(-95.0)
    assert listing.address == "123 Main St SW"
    assert listing.city == "Example City"
...
@pytest.mark.parametrize("text,street,city", [
    ("123 Main St SW, Example City, Alberta T2P1A1", "123 Main St SW", "Example City"),
    ("456 Elbow Dr NW, Example City", "456 Elbow Dr NW", "Example City"),
...
def test_alternate_field_paths_are_tolerated(source):
    ...
    record = {
        ...
        "Property": {
            ...
            "Address": {"AddressText": "9 Test St NW, Example City", "Latitude": "45.01",
                        "Longitude": "-95.01"},
        },
...
def test_health_check_warns_when_a_critical_field_is_mostly_absent(source, caplog):
    ...
    listings = [
        Listing(mls_id=f"X{i}", source="realtor_ca", url="", address="a",
                price=None, latitude=45.0, beds=3.0)
        for i in range(10)
    ]
...
def test_health_check_quiet_when_fields_are_present(source, caplog):
    ...
    listings = [
        Listing(mls_id=f"X{i}", source="realtor_ca", url="", address="a",
                price=700_000, latitude=45.0, beds=3.0)
        for i in range(10)
    ]
    with caplog.at_level(logging.WARNING):
        _check_field_health(listings)
