...
<table>
  <tr><td>
    <a href="https://www.realtor.ca/real-estate/28941234/123-main-st-sw-example-city-alberta">
  ...
  <tr><td>
    <a href="https://www.realtor.ca/real-estate/28955678/45-elbow-dr-nw-example-city-alberta">
...
def test_parses_listings_from_html():
    listings = parse_message(SAMPLE_HTML.encode(), area=AREA)
    ...
    assert first.url.endswith("123-main-st-sw-example-city-alberta")


def test_parses_listings_from_eml_with_headers():
    listings = parse_message(SAMPLE_EML.encode(), area=AREA)
    assert len(listings) == 2
...
def test_quoted_printable_soft_breaks_are_repaired():
    ...
    wrapped = (
        ...
        'sw-example-city">x</a><p>$725,000</p><p>3 Bedrooms 2 Bathrooms</p>'
    )
    listings = parse_message(wrapped.encode(), area=AREA)
...
def test_duplicate_links_collapse():
    ...
    assert len(parse_message(doubled.encode(), area=AREA)) == 2


def test_falls_back_to_url_id_when_no_mls_number():
    body = '<a href="https://www.realtor.ca/real-estate/28999999/9-test-st-se-example-city">x</a><p>$700,000</p>'
    listings = parse_message(body.encode(), area=AREA)
    assert listings[0].mls_id == "RCA28999999"


def test_french_urls_are_recognized():
    body = '<a href="https://www.realtor.ca/immobilier/28941299/7-rue-test-sw-example-city">x</a><p>$700,000</p>'
    assert len(parse_message(body.encode(), area=AREA)) == 1
...
@pytest.mark.parametrize("slug,expected", [
    ("123-main-st-sw-example-city-alberta", "123 Main St SW"),
    ("45-elbow-dr-nw-example-city", "45 Elbow Dr NW"),
    ("1010-5th-street-se-example-city-alberta-canada", "1010 5th Street SE"),
    ("9-deer-lane-rd-se-example-city", "9 Deer Lane Rd SE"),
])
def test_address_from_slug(slug, expected):
    assert _address_from_slug(slug, AREA) == expected


def test_address_from_slug_keeps_quadrant_uppercase():
    """The quadrant must stay uppercase — the assessment join is exact-match."""
    assert _address_from_slug("7-test-st-nw-example-city", AREA).endswith("NW")


def test_address_from_slug_empty():
    assert _address_from_slug("") == ""
    assert _address_from_slug("example-city-alberta", AREA) == ""
...
def test_reads_files_from_inbox(source, tmp_path):
    ...
    listings = source.fetch(SearchCriteria(area=AREA))
    assert len(listings) == 2
...
def test_later_email_supersedes_earlier_for_same_listing(source, tmp_path):
    ...
    listings = {l.mls_id: l for l in source.fetch(SearchCriteria(area=AREA))}
...
def test_unparseable_file_does_not_abort_the_run(source, tmp_path):
    ...
    assert len(source.fetch(SearchCriteria(area=AREA))) == 2
...
def test_list_date_taken_from_message_date():
    ...
    """
    listings = parse_message(SAMPLE_EML_DATED.encode(), area=AREA)
...
def test_message_date_survives_reconciliation_into_days_on_market():
    ...
    listing = parse_message(SAMPLE_EML_DATED.encode(), area=AREA)[0]
...
@pytest.mark.parametrize("address,expected", [
    ("71 Someplace Vi NW", "71 Someplace Villas NW"),
...
def test_expand_for_geocoding(address, expected):
    """The source municipality's own abbreviations are not resolvable by Nominatim.
